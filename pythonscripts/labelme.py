...
# genes are position codes 0 through 7 for each label
...
if len(sys.argv) == 6:
    ...
    print len(sys.argv)
    print "Usage: labelme.py filename image_width image_height label_offset font_size"
    quit()
...
def gen_rect(rect, code):
    ...
    width = r - l
    height = t - b
    xdisp = [-1, -1, -1,  0,  0,  1,  1,  1][code]*(width/2.0+label_offset)
    ydisp = [-1,  0,  1, -1,  1, -1,  0,  1][code]*(height/2.0+label_offset)
...
def point_intersect(rect, point):
    l, b, r, t = rect
    x, y = point
...
def objective(rects, gene, pointslist):
    ...
    for i in range(len(rectlist)):
        if abs(rect_intersect(rectlist[i], image_rect) < rect_intersect(rectlist[i], rectlist[i])):
            area += image_width * image_height * 10
    for i in range(len(pointslist)):
        if point_intersect(rectlist[i], pointslist[i]) == 1:
            area += image_width * image_height
...
gmtcodes=('TR','MR','BR','TC','BC','TL','ML','BL')
...
for i in range(n_labels):
    ...
    print("text is :{}: len is {} gmtcode is {}".format(text, len(text), gmtcode))
    width=1*font_size*len(text)
    height=1.25*font_size
    ax.scatter(x,y)
...
print "cover =", sum([rect_intersect(r,r) for r in rects])
...
bestgenecopy=bestgene
# for i in range(n_labels):
#     thisbest=bestgene
#     if bestgenecopy[i] != 7:
#         oldgene=bestgenecopy[i]
#         # Calculate the overlap area
#         areathen=objective(newrectlist, bestgenecopy)
#         bestgenecopy[i]=7
#         areanow=objective(newrectlist, bestgenecopy)
#         # If there is no change in area it is because no new overlap occurs
#         print "label {}: areathen={} areanow={}".format(i, areathen, areanow)
#
#         # If the area is different before and after, keep the old gene
#         if (areathen!=areanow):
#             bestgenecopy[i]=oldgene
...
for i in range(n_labels):
    ...
    text=d[i][7]
    gmtcode=gmtcodes.index(d[i][6])
    print("text is :{}:, old gmtcode is {} ({}), new gmtcode is {} ({})".format(text, gmtcode, d[i][6], bestgene[i], gmtcodes[bestgene[i]]))
...
with open('newlabels.txt', 'w') as f:
    ...
    for thisrect in fixedrectlist:
        addpatch_Rectangle_lbtr(thisrect, ax, 'brown')
        # print("Label {} has new justification code {}".format(index, gmtcodes[bestgenecopy[index]]))
        print("{}\t{}\t{}\t{}\t{}\t{}".format(d[index][2], d[index][3], d[index][4], d[index][5], gmtcodes[bestgene[index]], d[index][7]))
        index=index+1
