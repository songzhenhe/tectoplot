...
/*
  Calculate the shift.
*/
      scale = fmax ( fabs ( s[mn-1] ), 
              fmax ( fabs ( s[mn-2] ), 
              fmax ( fabs ( e[mn-2] ), 
              fmax ( fabs ( s[l-1] ), fabs ( e[l-1] ) ) ) ) );
...
/*
  ...
  Discussion:
    ...
    "free()" statements at the end, 08 April 2013.
...
*/
{
  ...
  lda = m;
  tol = DBL_EPSILON / r8mat_amax ( m, n, a_qr );
...
*/
  ...
  lda = m;
  sdiag = ( double * ) malloc ( i4_max ( m + 1, n ) * sizeof ( double ) );
  e = ( double * ) malloc ( i4_max ( m + 1, n )* sizeof ( double ) );
