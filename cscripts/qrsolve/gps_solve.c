...
int main ( ) {
    ...
    // m     --   integer indicating number of rows
    // n     --   integer indicating number of columns
    // b1    --   vector b with m rows
    // b2
    // ..
    // bn
    // a1,1  --   matrix a with m*n entries in row-column order
    // a1,2
    // ...
    // a1,n
    // a2,1
    ...
    // fill matrix a in the order expected by the least squares routine
    for ( i = 0; i < m; i++ ) {
      for ( j = 0; j < n; j++ ) {
        scanf("%lf", &a[i+j*m]);
      }
    ...
    // r2 = r8mat_mv_new ( m, n, a, x );
    // for ( i = 0; i < m; i++ ) {
    //     fprintf(stdout, "%g ", r2[i] );
    //     r2[i] = r2[i] - b[i];
    // }
