...
# Updated to use geojson as the conversion format instead of SHP
...
function tectoplot_calculate_shapefile_line()  {
  ...
  # This will redo the calculation for each call to _calculate_ I believe
  ...
  for ship_ind in $(seq 1 ${#m_shapefile_line_clipflag}); do
    if [[ ${m_shapefile_line_clipflag} -eq 1 ]]; then
      ...
      ogr2ogr -f geojson -skipfailures -t_srs "EPSG:4326" -clipsrclayer clip.txt ./shapefile_line/clip_${ship_ind}.geojson ${m_shapefile_line_file[$ship_ind]} >/dev/null 2>&1
      m_shapefile_line_file[$ship_ind]=./shapefile_line/clip_${ship_ind}.geojson
    fi
  done
  ...
  m_shapefile_shp_l)
    ...
    if [[ ${m_shapefile_line_data[$tt]} != "none" ]]; then
      ...
      case ${m_shapefile_line_minmax[$tt]} in
        region)
          [[ -s ./shapefile_line/clip_${tt}.geojson ]] && m_shapefile_line_selectfile=./shapefile_line/clip_${tt}.geojson
        ;;
        dataset)
          m_shapefile_line_selectfile=${m_shapefile_line_file[$tt]}
        ;;
    ...
    if [[ $(echo "${m_shapefile_line_widthbin[$tt]} > 0" | bc) -eq 1 && ${m_shapefile_line_data[$tt]} != "none" ]]; then
      ...
      for this_bin in $(seq 0 $((${m_shapefile_line_widthbin[$tt]} - 1 )) ); do
        ...
        # echo         ogr2ogr -sql "SELECT * from $(basename ${m_shapefile_line_file[$tt]} | sed 's/\(.*\)\..*/\1/') where ((${m_shapefile_line_data[$tt]} >= ${m_shapefile_lowbound}) and (${m_shapefile_line_data[$tt]} < ${m_shapefile_upbound}))" ./shapefile_line/selected_${tt}_${this_bin}.geojson ${m_shapefile_line_file[$tt]}

        ogr2ogr -f geojson -sql "SELECT * from $(basename ${m_shapefile_line_file[$tt]} | sed 's/\(.*\)\..*/\1/') where ((${m_shapefile_line_data[$tt]} >= ${m_shapefile_lowbound[$this_bin]}) and (${m_shapefile_line_data[$tt]} < ${m_shapefile_upbound[$this_bin]}))" ./shapefile_line/selected_${tt}_${this_bin}.geojson ${m_shapefile_line_file[$tt]}

        gmt psxy ./shapefile_line/selected_${tt}_${this_bin}.geojson -W${m_shapefile_line_thiswidth[$tt,$this_bin]}p,${m_shapefile_line_color[$tt]} ${m_shapefile_line_cptcmd} --PS_LINE_CAP=round --PS_LINE_JOIN=round $RJOK $VERBOSE >> map.ps
  # echo gmt psxy ./shapefile_line/selected_${tt}_${this_bin}.geojson -W${m_shapefile_line_thiswidth}p,${m_shapefile_line_color[$tt]} ${m_shapefile_line_cptcmd} --PS_LINE_CAP=round --PS_LINE_JOIN=round $RJOK $VERBOSE \>\> map.ps
